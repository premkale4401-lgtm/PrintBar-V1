...
@router.post(
    "/register", status_code=status.HTTP_201_CREATED, summary="Admin: Provision a new kiosk"
)
async def register_kiosk(
    ...
    logger.info(
        "kiosk_registered", kiosk_id=str(kiosk.id), name=body.name, admin_id=str(current_user.id)
    )
    return JSONResponse(
        ...
        content={
            ...
            "data": {
                ...
                "city": kiosk.city,
                "apiKey": raw_key,
                "warning": "This API key will NOT be shown again. Configure it in kiosk.yaml immediately.",
                "createdAt": str(kiosk.created_at) if kiosk.created_at else None,
            },
        },
    )
...
async def authenticate_kiosk(
    body: KioskAuthRequest, db: AsyncSession = Depends(get_db)
) -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={
                "success": False,
                "error": {"code": "KSK_001", "message": "Invalid kiosk ID format."},
            },
        )
    repo = KioskRepository(db)
    ...
    if kiosk is None or kiosk.id != kiosk_uuid:
        ...
        return JSONResponse(
            ...
            content={
                "success": False,
                "error": {"code": "KSK_002", "message": "Invalid kiosk credentials."},
            },
        )
...
async def kiosk_heartbeat_http(
    body: KioskHeartbeatRequest, db: AsyncSession = Depends(get_db)
) -> JSONResponse:
    ...
    if not kiosk:
        return JSONResponse(
            ...
            content={"success": False, "error": {"code": "KSK_003", "message": "Kiosk not found."}},
        )
    await repo.update_heartbeat(
        kiosk_uuid,
        status="PRINTING" if body.printing else "ONLINE",
        app_version=body.app_version,
        cpu_percent=body.cpu_percent,
        ram_percent=body.ram_percent,
        disk_percent=body.disk_percent,
        temperature_c=body.temperature_c,
        printer_status=body.printer_status,
    )
    ...
    return JSONResponse(
        ...
        content={
            "success": True,
            "data": {"kioskId": body.kiosk_id, "serverTime": datetime.now(tz=UTC).isoformat()},
        },
    )
    ...
    return JSONResponse(
        ...
        content={
            ...
            "data": {
                ...
                "wsConnected": kiosk.ws_connected,
